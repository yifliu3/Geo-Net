...
def fps_weight(x, k, weight=None):
    assert weight is not None, "the weight should be defined if using weighted fps"
    b, n, _ = x.shape
    x = x.view(-1, 3)
    ...
    offset = torch.cumsum(offset, dim=0).int()
    new_offset = torch.cumsum(new_offset, dim=0).int()
    idx = furthestsampling_weight(x, offset, new_offset, weight).long()
    return x[idx].view(b, k, 3)
...
class FurthestSamplingWeight(Function):
    @staticmethod
    def forward(ctx, xyz, offset, new_offset, weights):
        """
        input: xyz: (n, 3), offset: (b), new_offset: (b)
        output: idx: (m)
        """
        ...
        idx = torch.cuda.IntTensor(new_offset[b-1].item()).zero_()
        tmp = torch.cuda.FloatTensor(n).fill_(1e10)
        pointops_cuda.furthestsampling_weights_cuda(b, n_max, xyz, offset, new_offset, weights, tmp, idx)
        del tmp
        return idx

furthestsampling_weight = FurthestSamplingWeight.apply
