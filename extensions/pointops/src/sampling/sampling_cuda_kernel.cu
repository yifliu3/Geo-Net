...
template <unsigned int block_size>
__global__ void furthestsampling_weights_cuda_kernel(const float *xyz, const int *offset, const int *new_offset, const float *weights, float *tmp, int *idx)
{
    __shared__ float dists[block_size];
    __shared__ int dists_i[block_size];
    ...
    for (int j = start_m + 1; j < end_m; j++)
    {
        ...
        for (int k = start_n + tid; k < end_n; k += stride)
        {
            if (j == 0) {
                ...
                // float d2 = d * max(weights[k], 1e-12);
                // besti = d2 > best ? k : besti;
                // best = d2 > best ? d2 : best;
            }
...
{   
	...
	switch (n_threads) {
        case 1024:
            furthestsampling_weights_cuda_kernel<1024><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 512:
            furthestsampling_weights_cuda_kernel<512><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 256:
            furthestsampling_weights_cuda_kernel<256><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 128:
            furthestsampling_weights_cuda_kernel<128><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 64:
            furthestsampling_weights_cuda_kernel<64><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 32:
            furthestsampling_weights_cuda_kernel<32><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 16:
            furthestsampling_weights_cuda_kernel<16><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 8:
            furthestsampling_weights_cuda_kernel<8><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 4:
            furthestsampling_weights_cuda_kernel<4><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 2:
            furthestsampling_weights_cuda_kernel<2><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        case 1:
            furthestsampling_weights_cuda_kernel<1><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
            break;
        default:
            furthestsampling_weights_cuda_kernel<512><<<b, n_threads, 0>>>(xyz, offset, new_offset, weights, tmp, idx);
    }
