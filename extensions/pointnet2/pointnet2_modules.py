...
class PointnetSAModuleVotes_nofps(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        #         xyz, new_xyz, features
        #     )  # (B, C, npoint, nsample), (B,3,npoint,nsample), (B,npoint) org B G M 3
        new_xyz = xyz
        grouped_features = xyz.permute(0,3, 1, 2)
        grouped_xyz =  unique_cnt = grouped_features
...
class PointnetSAModuleVotes_nogrouping(nn.Module):
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        features = torch.cat(
                    [torch.zeros_like(new_xyz.transpose(1, 2)), features], dim=1
                )
...
class PointnetSAModuleMSGVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
                features: torch.Tensor = None, inds: torch.Tensor = None):
        r"""
        Parameters
        ----------
        xyz : torch.Tensor
